    ...
    override func spec() {
        ...
        beforeEach {
            ...
            let endpointClosure: MoyaProvider<HTTPBin>.EndpointClosure = { target in
                ...
                return Endpoint(url: URL(target: target).absoluteString, sampleResponseClosure: {.networkResponse(200, target.sampleData)}, underlyingTarget: target, method: target.method, task: task, httpHeaderFields: target.headers)
            }
