...
public extension AnyPublisher where Output == Response, Failure == MoyaError {
    ...
    func filter<R: RangeExpression>(statusCodes: R) -> some Publisher<Response, MoyaError> where R.Bound == Int {
    ...
    func filter(statusCode: Int) -> some Publisher<Response, MoyaError> {
        return unwrapThrowable { response in
    ...
    func filterSuccessfulStatusCodes() -> some Publisher<Response, MoyaError> {
        return unwrapThrowable { response in
    ...
    func filterSuccessfulStatusAndRedirectCodes() -> some Publisher<Response, MoyaError> {
        return unwrapThrowable { response in
    ...
    func mapImage() -> some Publisher<Image, MoyaError> {
        return unwrapThrowable { response in
    ...
    func mapJSON(failsOnEmptyData: Bool = true) -> some Publisher<Any, MoyaError> {
        return unwrapThrowable { response in
    ...
    func mapString(atKeyPath keyPath: String? = nil) -> some Publisher<String, MoyaError> {
        return unwrapThrowable { response in
    ...
    func map<D: Decodable>(_ type: D.Type, atKeyPath keyPath: String? = nil, using decoder: JSONDecoder = JSONDecoder(), failsOnEmptyData: Bool = true) -> some Publisher<D, MoyaError> {
        return unwrapThrowable { response in
...
public extension AnyPublisher where Output == ProgressResponse, Failure == MoyaError {
     ...
     */
    func filterCompleted() -> some Publisher<Response, MoyaError> {
        return self
     ...
     */
    func filterProgress() -> some Publisher<Double, MoyaError> {
        return self
            .filter { !$0.completed }
            .map { $0.progress }
    }
...
extension AnyPublisher where Failure == MoyaError {
    ...
    private func unwrapThrowable<T>(throwable: @escaping (Output) throws -> T) -> some Publisher<T, MoyaError> {
