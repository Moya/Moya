...
你是个聪明的开发者。你可能使用 [Alamofire](https://github.com/Alamofire/Alamofire) 来抽象对 `URLSession` 的访问，以及所有那些你并不真正关心的糟糕细节。但是，就像许多聪明开发者一样，你编写专有的网络抽象层，它们可能被称作 "APIManager" 或 "NetworkModel"，并且它们总是以眼泪结束。
...
所以，Moya 的基本思想是，提供一些网络抽象层，它们直接对 Alamofire 进行充分地封装。它们应该足够简单，可以很容易地应付常见的任务，也应该足够全面，应付复杂的任务也同样容易。
...
这个项目正在积极的开发中，并且它正被用于 [Artsy's new auction app](https://github.com/Artsy/eidolon)。我们认为它已经可以用于生产环境了。

## 安装

### Moya 版本 vs Swift 版本

下面显示了 Moya 版本与其对应的 Swift 版本。
...
然后指定 `"Moay"` 为你想要使用 Moya 的 Target 的依赖项。
...
需要注意的是，从 Moya 10 开始，SPM 仅适用于 Swift 4 或更高版本的工具链。
...
在任何你想使用 Moya 的文件中，通过 `import Moya` 来导入框架。

### Carthage

Carthage 用户可以指向这个仓库然后使用他们想要的任何一个生成的框架，无论是 `Moya`，`RxMoya` 或者 `ReactiveMoya`。
...
如果这是你首次在项目中使用 Carthage，那么你需要进行一些额外的步骤，它们在 [over at Carthage](https://github.com/Carthage/Carthage#adding-frameworks-to-an-application) 中有解释。

### 手动

- 打开终端，`cd` 到你项目的顶层目录，如果你的项目没有初始化 git 作为仓库，然后运行下面的命令：

```bash
$ git init
```

- 通过运行以下命令来添加 Alamofire，Result & Moya 作为 git 子模块 [submodule](http://git-scm.com/docs/git-submodule)：
...
- 打开新建的 `Alamofire` 文件夹，并且把 `Alamofire.xcodeproj` 拖拽到你 XCode 项目中的 Project Navigator 里。对 Result 文件夹下的 `Result.xcodeproj` 和 Moya 文件夹下的 `Moya.xcodeproj` 做同样的操作。

> 它们应该嵌套在应用程序的蓝色项目图标下面，在其它 Xcode group 的上面或者下面都没关系。

- 核查 `xcodeprojs` 的部署目标与你项目导航器中的应用程序目标一致。
...
在 [一些设置](https://github.com/Moya/Moya/blob/master/docs_CN/Examples/Basic.md) 之后，使用 Moya 相当地简单。你可以像下面的方式访问 API：
...
这个一个基本的示例。很多 API 请求都需要参数。Moya 将参数编码到 enum 中，来访问端点，就像这样：
...
URLs 不再有书写错误，不再有缺失参数值，也不再有混乱的参数编码。
...
除了使用信号而不是回调闭包之外，RxSwift 和 ReactiveSwift 还有一系列信号操作符，它们可以把从网络响应接收到的数据分别通过 `mapImage()`，`mapJSON()` 以及 `mapString()` 映射成一个图片、一些 json 或者一个字符串。如果映射不成功，你会从信号中得到一个错误。你还可以使用一些方便的方法来过滤某些状态码。这意味着你可以将处理 API 错误（比如 400）的代码与处理无效响应的代码放置在相同的位置上。
...
Moya 社区拥有巨大的正能量，并且维护人员致力于让事情变得更棒。像 [CocoaPods](https://github.com/CocoaPods/CocoaPods/wiki/Communication-&-Design-Rules) 一样，总是提取积极的意图；即使某个评论听起来非常刻薄，它仍会让人从怀疑中受益。
...
如果你从 Moya 添加或者移除一个源文件，相应的改变需要在这个仓库的根目录的 Moya.xcodeproj 中同步。这个项目要用于 Carthage。但是别担心，如果你提交请求时忘了，会收到一个自动的警告。
...
Moya 是在 MIT license 下发布的。更多信息查看 [License.md](https://github.com/Moya/Moya/blob/master/License.md)。
